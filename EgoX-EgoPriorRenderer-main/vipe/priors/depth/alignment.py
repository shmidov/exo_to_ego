...
def align_metric_depth_to_depth(
    source_depth: torch.Tensor,
    target_depth: torch.Tensor,
    target_mask: torch.Tensor | None = None,
    quantile_masking: bool = True,
    ...
    """
    Apply affine transformation to align source metric depth to target depth.
    Similar to align_inv_depth_to_depth but without taking reciprocal of target depth.

    Args:
        source_depth: Metric depth map to be aligned. Shape: (H, W).
        target_depth: Target depth map. Shape: (H, W).
        target_mask: Mask of valid target pixels. Shape: (H, W).
