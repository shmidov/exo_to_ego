...
class Trainer:
    ...
    def prepare_for_validation(self):
        ...
        # IC-LoRA specific validation data
        if self.args.model_type == "wan-i2v":
            if self.args.validation_exo_videos is not None:
                validation_exo_videos = load_videos(self.args.validation_dir / self.args.validation_exo_videos)
            else:
                validation_exo_videos = [None] * len(validation_prompts)

            if self.args.validation_ego_prior_videos is not None:
                validation_ego_prior_videos = load_videos(self.args.validation_dir / self.args.validation_ego_prior_videos)
            else:
                validation_ego_prior_videos = [None] * len(validation_prompts)

            self.state.validation_prompts = validation_prompts
            self.state.validation_exo_videos = validation_exo_videos
            self.state.validation_ego_prior_videos = validation_ego_prior_videos
        else:
            # Legacy validation for other model types
    ...
    def validate(self, step: int) -> None:
        ...
        for i in range(num_validation_samples):
            ...
            # Initialize placeholders so variables exist in all branches
            image = None
            video = None

            if self.args.model_type == "wan-i2v":
                # IC-LoRA validation data
                exo_video = self.state.validation_exo_videos[i]
                ego_prior_video = self.state.validation_ego_prior_videos[i]
                
                if exo_video is not None:
                    exo_video = preprocess_video_with_resize(exo_video, self.state.train_frames, self.state.train_height, self.state.train_width, "exo")
                    # Convert video tensor (F, C, H, W) to PIL images
                    exo_video = [Image.fromarray((frame.permute(1, 2, 0).numpy() * 255).astype(np.uint8)) for frame in exo_video]
                
                if ego_prior_video is not None:
                    ego_prior_video = preprocess_video_with_resize(ego_prior_video, self.state.train_frames, self.state.train_height, self.state.train_width, "ego_prior")
                    # Convert video tensor (F, C, H, W) to PIL images
                    ego_prior_video = [Image.fromarray((frame.permute(1, 2, 0).numpy() * 255).astype(np.uint8)) for frame in ego_prior_video]
            else:
            ...
            if self.args.model_type == "wan-i2v":
                # IC-LoRA validation
                validation_artifacts = self.validation_step({"prompt": prompt, "exo_video": exo_video, "ego_prior_video": ego_prior_video}, pipe)
            else:
                # Legacy validation
